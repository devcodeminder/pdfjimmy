...
import 'dart:io';
import 'dart:math';
import 'package:pdfjimmy/core/models/reading_stats.dart';
...
class PdfTtsService {
  ...
  Future<void> _initializeTts() async {
    try {
      ...
      // Handle completion universally. If awaitSpeakCompletion is true,
      // the handler gets triggered correctly on most platforms. 
  ...
  Future<void> _speakNextSentence() async {
    ...
    try {
      ...
      final result = await _tts.speak(textToSpeakFinal);
      print('PdfTtsService: ✅ FINISHED speaking sentence. Result: $result');

      // Platform-specific handling
      if (Platform.isWindows) {
        // On Windows, await completes when speaking finishes
        print('PdfTtsService: Windows TTS completed, advancing to next sentence');
        
        // If result is 0 (failure), wait a bit so we don't infinitely fast-loop which 
        // causes the "auto scroll without reading" bug when TTS engine fails to speak.
        if (result == 0) {
           print('PdfTtsService: Windows TTS failed to speak. Adding delay to prevent fast-loop.');
  ...
  Future<void> seekToTapPosition(
    ...
    int targetPageNumber = (pageIndex != null) ? pageIndex + 1 : (pageNumber ?? _currentPageNumber);

    // If no sentences are loaded OR the target page is different from the loaded page:
    if (_sentences.isEmpty || targetPageNumber != _currentPageNumber) {
      if (filePath != null) {
        print(
          'PdfTtsService: seekToTapPosition — loading target page $targetPageNumber before seek',
        );
        _isInitializing = true;
        try {
          _currentPageNumber = targetPageNumber;
          _totalPages = totalPages ?? _totalPages;
          await _extractTextAndBounds(filePath, targetPageNumber);
        ...
        print(
          'PdfTtsService: seekToTapPosition — sentences empty/mismatch and no file info provided',
        );
        return;
      }
    }
