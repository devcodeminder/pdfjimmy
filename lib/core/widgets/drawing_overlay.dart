...
class DrawingOverlay extends StatefulWidget {
  ...
  final Function(DrawingPath)? onMagicPenDrawn; // Callback for magic pen lasso
...
class _DrawingOverlayState extends State<DrawingOverlay> {
  ...
  void _onPanStart(DragStartDetails details) {
    if (widget.drawingMode == DrawingMode.draw ||
        widget.drawingMode == DrawingMode.magicPen) {
      setState(() {
        _currentPath = DrawingPath(
          filePath: widget.filePath,
          pageNumber: widget.pageNumber,
          points: [details.localPosition],
          color: widget.drawingMode == DrawingMode.magicPen
              ? const Color(0xFF00FFCC).withValues(alpha: 0.7)
              : widget.selectedColor,
          strokeWidth: widget.drawingMode == DrawingMode.magicPen
              ? 6.0
              : widget.strokeWidth,
        );
      });
  ...
  void _onPanEnd(DragEndDetails details) {
    if (widget.drawingMode == DrawingMode.draw ||
      ...
      if (_currentPath != null && _currentPath!.points.length > 1) {
        if (widget.drawingMode == DrawingMode.magicPen) {
          widget.onMagicPenDrawn?.call(_currentPath!);
        } else {
