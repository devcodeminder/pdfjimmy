  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // Calculate the actual position where the magnifier is placed
        final double magWidth = 120;
        final double magHeight = 120;
        final double magVerticalOffset = 160;

        final double magLeft = (_magnifierPosition!.dx - magWidth / 2).clamp(
          0,
          viewSize.width - magWidth,
        );
        final double magTop = (_magnifierPosition!.dy - magVerticalOffset)
            .clamp(0, viewSize.height - magHeight);

        // The focalPointOffset is the shift from the magnifier's window center (magLeft + 60, magTop + 60)
        // to the actual finger position (_magnifierPosition).
        final focalOffset =
            _magnifierPosition! -
