...
class PdfController extends ChangeNotifier {
  ...
  // ✅ Debounce timer — prevents DB write on every scroll page change
  ...
  int _customColorValue = GetStorage().read('customColor') ?? 0xFF0280F8;

  static const List<Color> themeColors = [
    Color(0xFF0280F8), // Indigo (Default)
  ...
  void setCurrentPage(int page) {
    _currentPage = page;
    // ✅ Debounce: only write to DB after the user stops on a page for 1 second
    if (_currentFilePath != null) {
      _pageUpdateDebounce?.cancel();
      _pageUpdateDebounce = Timer(const Duration(seconds: 1), () {
