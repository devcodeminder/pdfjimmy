...
class _AITranslationScreenState extends State<AITranslationScreen> {
  String _selectedLanguage = 'ta'; // Tamil by default
  ...
  Future<void> _saveTranslatedPdf() async {
    ...
    try {
      ...
      // Create pdfjimmy folder if helpful, else just drop in Download
      final finalDir = Directory('${dir!.path}/PDFJimmy');
      if (!await finalDir.exists()) {
        await finalDir.create(recursive: true);
      }
      
      final path = '${finalDir.path}/$name';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        fit: StackFit.expand,
        children: [
          // 1. Content (PDF Pages or Image)
          _buildBody(),

          // 2. Top Premium Glassmorphism HUD
          Positioned(
            top: 0,
            left: 0,
            right: 0,
                    ...
                    child: Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text(
                            _totalPages > 0
                                ? 'P${_currentPageIndex + 1}/$_totalPages'
                                : 'Auto',
    ...
    if (_isSimpleMode && _simpleTranslatedText != null) {
      ...
      return SingleChildScrollView(
        ...
        child: SelectableText(
          _showOriginal ? (_simpleOriginalText ?? "No text found") : _simpleTranslatedText!,
        ),
      );
    }
